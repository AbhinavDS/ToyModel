...
class Splitter(nn.Module):
	...
	def calculateReward(points, c, Pid, gt, mask):
		p1, q1, p2, q2 = points
		batch_size = c.size(0)
		reward = torch.zeros((batch_size),dtype=dtype)
		num_intersections = 0
		edges = []
		for b in range(batch_size):
			...
			if num_intersections != 2:
				reward[b] = 0
				continue
			else if Pid[b][edges[0][0],edges[0][1]] != Pid[b][edges[1][0],edges[1][1]]:
				reward[b] = 0
				continue
			else:
				...
				for i in range(masked_gt.size(0)):
					if masked_gt[i,0].item() == -2:
						continue
					x1, y1, x2, y2 = masked_gt[i,0].item(), masked_gt[i,1].item(), masked_gt[(i+1)%masked_gt.size(0),0].item(), masked_gt[(i+1)%masked_gt.size(0),0].item()
