...
def train_model(params):
	...
	num_blocks = int(math.ceil(np.log2(max_vertices))) - 2 # - 2 #(since we start with 3 vertices already)
	params.num_blocks1 = num_blocks
	params.num_blocks2 = 1
	...
	for epoch in range(params.num_epochs):
		...
		epoch_set = int(params.num_epochs/(2*(params.num_polygons)))# 1000
		epoch_id = int(epoch/epoch_set)
		condition_train1 = (epoch_id == 0)
		...
		for i in range(int(math.ceil(data_size/params.batch_size))):
			...
			x, c, s, A, Pid, proj_pred = model.forward1(x, c, s, A, Pid, gt, gtnormals, loss_mask)
			...
			if not condition_train1:#[1,2,3,4,5]
				...
				for block_i in range(train_till_block):
					max_block_i = int((int(params.num_epochs/epoch_set) + 1)/2)-1
					is_last_block = (block_i ==  max_block_i)#3
					action, reward, intersections, pred_genus, gt_genus = model.split(c, x, gt, Pid, mask, proj_pred, proj_gt, epoch * params.data_size + i, test_rl or  epoch_id%2 == 0 or (block_i<train_till_block-1), block_i, to_split = not is_last_block)
					...
					if is_last_block:
						...
						if not(epoch_id%2==1 and block_i == train_till_block-1):
							x, c, s, A, Pid,  proj_pred = model.forward2(x.detach(), c.detach(), s.detach(), A, Pid, gt, gtnormals, loss_mask)
							total_closs += model.closs/len(train_data)
							total_laploss += model.laploss/len(train_data)
							total_nloss += model.nloss/len(train_data)
							total_eloss += model.eloss/len(train_data)
					...
					if block_i == train_till_block-1:
						color = 'red' if (epoch_id%2==0 or is_last_block) else 'blue'
						utils.drawPolygons(utils.getPixels(c[0]),utils.getPixels(masked_gt),proj_pred=proj_pred[0], proj_gt=proj_gt[0], color=color,out='results/pred_rl%s.png'%params.sf,A=A[0], line=(action[0][0],action[0][1],action[0][2],action[0][3]))

				if epoch_id%2 == 0 and train_till_block < int((int(params.num_epochs/epoch_set) + 1)/2):#3
					optimizer.step()
			...
			if (iter_count % params.show_stat == 0) and (condition_train1) and epoch_id%2 == 0 :
				masked_gt = gt[0].masked_select(mask[0].unsqueeze(1).repeat(1,dim_size)).reshape(-1, dim_size)
		...
		end_time = time.time()
		if condition_train1 or epoch_id%2 == 0 :
